...
for j=1:10;
    ...
    k=1.1;
    for i=1:20
        if k>0.1001 % using 0.11 is crucial since matlab has a weird behaviour if 0.1 is used
            k=k-0.1;
        else
            k=k-0.01;
        end
...
% (considering sig_kstest result: n>=2 -observed number of co-annotated genes- and 
% k>=0.1 -co-occurrence similarity thereshold- was observed to be sufficient to assume significance)
...
high_det_thres=HPO_GO_mapping_sematic_sim_high_det(HPO_GO_mapping_sematic_sim_high_det(:,2)>=k,:);
...
% (using the parameters: n>=2 and k>=0.1 as previously obtained from randomization test)

n=2;
k=0.1;

high_det_thres=CAFA2_HPO_GO_mapping_sematic_sim_high_det(CAFA2_HPO_GO_mapping_sematic_sim_high_det(:,2)>=k,:);
high_det_thres=high_det_thres(high_det_thres(:,3)>=n,:);
...
pred=cafa_import('HPO2GO_Files/CAFA2_test_n2_S01/CAFA2_HPO_target_predictions.txt', HPO, false);
...
benchmark2=pfp_loaditem('CAFA2/CAFA2-master/benchmark/groundtruth/leafonly_HPO.txt', 'char');
fmax2 = pfp_seqmetric(benchmark2, pred, oa, 'fmax');
wfmax2 = pfp_seqmetric(benchmark2, pred, oa, 'wfmax', 'w', eia);
smin2 = pfp_seqmetric(benchmark2, pred, oa, 'smin', 'w', eia);
pr2 = pfp_seqmetric(benchmark2, pred, oa, 'pr');
save HPO2GO_Files/HPO2GO_CAFA2_perf_results.mat fmax fmax2 wfmax wfmax2 smin smin2 pr pr2 -v7

% (fmax=0.402, wfmax=0.319 the best performance in CAFA2)
